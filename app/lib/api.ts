 ...
 * nothing else in the frontend hardcodes a URL.
 */
export const endpoints = {
  ...
  /** Kept as an alias: the Django app is `news`, the route is `/articles/`. */
  news: "/articles/",
  articles: "/articles/",
  ...
  /**
   * NOTE: the backend has no newsletter endpoint yet, so `NewsletterForm`
   * currently fails. Either add a subscriber model + route to the Django
   * project, or drop the form.
   */
 ...
 * (the backend answers 404 until then — expected, not an error).
 */
